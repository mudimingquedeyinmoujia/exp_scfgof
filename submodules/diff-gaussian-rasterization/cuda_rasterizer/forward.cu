...
__device__ float3 computeCov2DOri(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* cov3D, const float* viewmatrix)
{
	...
	bool prefiltered)
{
	...
	// Compute 2D screen-space covariance matrix
	float3 cov = computeCov2DOri(p_orig, focal_x, focal_y, tan_fovx, tan_fovy, cov3D, viewmatrix);
